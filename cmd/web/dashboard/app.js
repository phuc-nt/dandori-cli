        ...
        function onSprintChange(value) {
            ...
            if (qaOpen) window.loadQAView?.();
            if (auditOpen) window.loadAuditView?.();
        }
        ...
        function showPersonaSection(persona) {
            ...
            if (persona === 'qa') window.loadQAView?.();
            if (persona === 'audit') window.loadAuditView?.();
        }
        ...
        // Moved to ES6 modules in Phase 05 / Step 03:
        //   /static/widgets/qa.js     → loadQAView, renderQA*
        //   /static/widgets/audit.js  → loadAuditView, reloadEventStream,
        //                                loadAuditLog, verifyAuditChain
        // The bootstrap module in index.html assigns them to window so
        // inline onclick="..." attributes (Refresh / Verify buttons) and
        // the persona-switch callers above keep working unchanged.
