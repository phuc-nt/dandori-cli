...
func TestLegacyBucketReworkReason_TableDriven(t *testing.T) {
	cases := []struct {
		...
		// v0.14 — intervention buckets (canonical, hyphen, space).
		{"wrong_approach", ReasonWrongApproach},
		{"wrong-approach", ReasonWrongApproach},
		{"WRONG APPROACH", ReasonWrongApproach},
		{"scope_misunderstanding", ReasonScopeMisunderstanding},
		{"scope misunderstanding", ReasonScopeMisunderstanding},
		{"missing_context", ReasonMissingContext},
		{"missing context", ReasonMissingContext},
