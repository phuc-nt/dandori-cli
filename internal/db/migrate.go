...
func (l *LocalDB) Migrate() error {
	...
	if currentVersion == 5 {
		if _, err := l.Exec(MigrationV5ToV6); err != nil {
	...
	if currentVersion == 8 {
		if err := MigrateV8ToV9Data(l); err != nil {
			return fmt.Errorf("migrate v8 to v9 (data): %w", err)
		}
		if _, err := l.Exec(MigrationV8ToV9); err != nil {
			return fmt.Errorf("migrate v8 to v9 (version): %w", err)
		}
		currentVersion = 9
	}
	if currentVersion == 9 {
		if _, err := l.Exec(MigrationV9ToV10); err != nil {
			return fmt.Errorf("migrate v9 to v10: %w", err)
		}
		currentVersion = 10
	}
	if currentVersion == 10 {
		if _, err := l.Exec(MigrationV10ToV11); err != nil {
			return fmt.Errorf("migrate v10 to v11: %w", err)
		}
		currentVersion = 11
	}
	if currentVersion == 11 {
		if _, err := l.Exec(MigrationV11ToV12); err != nil {
			return fmt.Errorf("migrate v11 to v12: %w", err)
		}
		currentVersion = 12
	}
	if currentVersion == 12 {
		if err := migrateV12ToV13(l); err != nil {
